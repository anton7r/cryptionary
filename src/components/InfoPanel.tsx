...
const isEmpty = (store: CoinStore) => store == null || store.priceHistory == null || store.priceHistory.length === 0;

const longestBear = (prices: CoinData[], startTime: number): number => {
    /*
    let lastDay = 0;
    prices = prices.filter((stamped) => {
...
const getHighest = (arr: CoinData[], after: number): GetHighest => {
    ...
    return {
        filteredArr: arr, 
        highest: arr.reduce((prev, curr) => curr.price > prev.price ? curr : prev)
    };
}
...
const timeMachine = (prices: CoinData[], startTime: number): string => {
    ...
    let { filteredArr: filteredHigh , highest } = getHighest(highPoints, firstLow.stamp);  

    const buyNSell: BuyNSell = lowPoints.map((lowPoint): BuyNSell => {
        ...
        return {buy: lowPoint, sell: highest, profitMultip: highest.price / lowPoint.price}
    }).reduce((prev, curr) => curr.profitMultip > prev.profitMultip ? curr : prev, undefined);
    ...
    return (
        <div class="infoPane">
            ...
            >
                ...
                <p>The longest bearish downward trend in the measurement period was { longestBear(props.store.priceHistory, props.startTime()) } days.</p>

                <h2>Highest volume</h2>
                <p>{ highestVolume(props.store.priceHistory, props.startTime()) }</p>

                <h2>Time machine</h2>
                <p>{ timeMachine(props.store.priceHistory, props.startTime()) }</p>
